...
@click.command()
@click.option('--config', '-c', type=click.Path(exists=True), default='agent_config.yaml',
...
def load_yaml_config(config_path: str, errors: List[str]) -> Optional[Dict[str, Any]]:
...
def validate_required_fields(config: Dict[str, Any], errors: List[str], warnings: List[str]):
    ...
    known_fields = {'agent', 'skills', 'routing', 'services', 'security', 'ai', 'mcp',
                   'middleware', 'monitoring', 'observability', 'development', 'registry_skills',
    ...
    for i, skill in enumerate(skills):
        ...
        if skill_id:
            ...
            if not re.match(r'^[a-zA-Z][a-zA-Z0-9_]*$', skill_id):
                errors.append(f"Invalid skill ID '{skill_id}'. Must start with letter and contain only letters, numbers, and underscores.")
...
def validate_middleware_config(middleware: List[Dict[str, Any]], skill_id: str,
    ...
    valid_middleware_types = {'rate_limit', 'cache', 'validation', 'retry',
                             'logging', 'timing', 'transform'}
