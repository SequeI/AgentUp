...
class SkillInstaller:
    ...
    async def update_skill(self, skill_id: str) -> bool:
        ...
        try:
            ...
            if current_version == latest_version:
                print(f"Skill {skill_id} is already up to date ({latest_version})")
                return True
            
            print(f"Updating {skill_id} from {current_version} to {latest_version}")
    ...
    async def enable_skill(self, skill_id: str) -> bool:
        ...
        if not self._is_in_agent_project():
            console.print(f"[red]❌ No agent project found[/red]")
    ...
    async def _copy_handler_to_project(self, skill_id: str, skill_dir: Path) -> bool:
        ...
        if not handler_file.exists():
            console.print(f"[red]❌ No handler.py found in skill package[/red]")
            return False
    ...
    def _show_install_preview(self, skill_info: Dict[str, Any], version: str) -> None:
        ...
        print(f"Would install: {skill_info['name']} v{version}")
        print(f"Description: {skill_info['description']}")
        print(f"Author: {skill_info['author']['display_name']}")
    ...
    def _show_integration_success_message(self, skill_id: str, version: str, skill_info: Dict[str, Any], 
        ...
        handler_path = self._get_handler_path(skill_id)
        console.print(f"  ✅ Handler: [cyan]{handler_path.relative_to(self.current_project)}[/cyan]")
        console.print(f"  ✅ Configuration: Added to [cyan]skills[/cyan] section")
        
        if deps_installed:
            console.print(f"  ✅ Dependencies: Installed to project")
  ...
  enabled: true"""
    ...
    async def _remove_from_agent_config(self, skill_id: str) -> None:
        ...
        try:
            ...
            print(f"Warning: Could not update agent_config.yaml: {e}")
    ...
    async def _update_skill_status(self, skill_id: str, enabled: bool) -> bool:
        ...
        if not config_path.exists():
            print("No agent_config.yaml found")
            return False
        ...
        try:
            ...
            if updated:
                ...
                print(f"Skill {skill_id} not found in configuration")
                return False
                
        except Exception as e:
            print(f"Error updating skill status: {e}")
            return False
