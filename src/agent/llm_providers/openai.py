...
class OpenAIProvider(BaseLLMService):
    ...
    async def initialize(self) -> None:
        ...
        if not self.api_key:
            logger.error(f"OpenAI service '{self.name}' initialization failed: API key not found. Check that 'api_key' is set in configuration and OPENAI_API_KEY environment variable is available.")
        ...
        try:
            ...
            raise LLMProviderError(f"Failed to initialize OpenAI service: {e}")
    ...
    async def chat_complete(self, messages: List[ChatMessage], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise LLMProviderAPIError(f"OpenAI API request failed: {e}")
        except KeyError as e:
            raise LLMProviderAPIError(f"Invalid OpenAI API response format: {e}")
    ...
    async def _embed_impl(self, text: str) -> List[float]:
        ...
        try:
            ...
            raise LLMProviderAPIError(f"OpenAI embeddings API request failed: {e}")
        except KeyError as e:
            raise LLMProviderAPIError(f"Invalid OpenAI embeddings API response format: {e}")
    ...
    async def stream_chat_complete(self, messages: List[ChatMessage], **kwargs):
        ...
        try:
            ...
            raise LLMProviderAPIError(f"OpenAI streaming API request failed: {e}")