"""Security manager for AgentUp authentication and authorization."""

import logging
from typing import Dict, Any, Optional, Set
from fastapi import Request, HTTPException

from .base import BaseAuthenticator, AuthenticationResult, SecurityPolicy
...
from .utils import log_security_event, get_request_info
...
class SecurityManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"Security manager initialized - enabled: {self.auth_enabled}, "
                   f"primary auth: {self.primary_auth_type}")
    ...
    def _initialize_authenticators(self) -> None:
        ...
        for auth_type in auth_types:
            try:
                ...
                raise SecurityConfigurationException(f"Failed to initialize {auth_type} authenticator: {e}")
        ...
        if not self.auth_enabled:
            if policy.require_authentication:
                log_security_event(
                    ...
                    request_info,
                    False,
                    "Authentication required but security is disabled"
                )
                raise HTTPException(
                    status_code=500,
                    detail="Authentication required but security is not configured"
                )
            return None
    ...
    async def _perform_authentication(
        ...
        if not policy.is_auth_type_allowed(target_auth_type):
            log_security_event(
                ...
                request_info,
                False,
                f"Auth type {target_auth_type} not allowed by policy"
            )
            raise HTTPException(
                status_code=403,
                detail=f"Authentication type '{target_auth_type}' not allowed"
            )
        ...
        if not authenticator:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Authenticator '{target_auth_type}' not available"
            )
        ...
        try:
            ...
            if policy.required_scopes and not policy.has_required_scopes(result.scopes):
                log_security_event(
                    ...
                    request_info,
                    False,
                    f"Insufficient scopes. Required: {policy.required_scopes}, "
                    f"User has: {result.scopes}"
                )
                raise HTTPException(
                    status_code=403,
