...
def assert_config_has_llm_service(config: Dict[str, Any], service_name: str, provider: str, model: str):
    ...
    assert service["provider"] == provider, f"LLM service provider mismatch. Expected '{provider}', got '{service['provider']}'"
...
class AgentConfigBuilder:
    ...
    def with_ollama_service(self, name: str = "ollama", model: str = "qwen3:0.6b"):
        ...
        return self.with_service(name, "llm", provider="ollama", 
                                base_url="${OLLAMA_BASE_URL:http://localhost:11434}", model=model)
