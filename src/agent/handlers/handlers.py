...
try:
    ...
    class ConversationContext:
        @classmethod
        def increment_message_count(cls, task_id): return 1
        @classmethod
        def get_message_count(cls, task_id): return 1

    class MessageProcessor:
        @staticmethod
        def extract_messages(task): return []
        @staticmethod
        def get_latest_user_message(messages): return None

    def extract_parameter(text, param): return None

# Optional middleware decorators (no-ops if unavailable)
try:
    from ..middleware import cached, rate_limited, logged, retryable, timed, with_middleware
except ImportError:
    def cached(ttl=300):
...
def get_handler(skill_id: str) -> Optional[Callable[[Task], str]]:
