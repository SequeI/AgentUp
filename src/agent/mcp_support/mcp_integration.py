...
async def _initialize_mcp_server(services, server_config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if port:
            logger.warning(f"Stdio MCP server on port {port} is not supported within FastAPI. Use HTTP MCP endpoint at /mcp instead.")
            ...
            if not registry.is_mcp_tool(function_name):  # Only register local functions
                ...
                if handler and schema:
                    ...
                    try:
                        ...
                        if "kwargs" in str(e):
                            logger.warning(f"Skipping function '{function_name}' - functions with **kwargs are not supported as MCP tools")
                        else:
                            logger.warning(f"Failed to register MCP tool '{function_name}': {e}")

        logger.info(f"Exposed {successful_registrations} handlers as MCP tools (out of {len(registry.list_functions())} total)")
