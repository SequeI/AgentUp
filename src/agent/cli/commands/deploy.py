"""Generate deployment files for agents."""
...
@click.command()
@click.option('--type', '-t', type=click.Choice(['docker', 'k8s', 'helm']),
              required=True, help='Deployment type to generate')
...
def deploy(type: str, output: Optional[str], port: int, replicas: int,
           image_name: Optional[str], image_tag: str):
...
def generate_k8s_files(output_dir: Path, agent_name: str, image_name: str,
                       image_tag: str, port: int, replicas: int):
