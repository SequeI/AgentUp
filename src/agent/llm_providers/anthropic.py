...
class AnthropicProvider(BaseLLMService):
    ...
    async def initialize(self) -> None:
        ...
        if not self.api_key:
            logger.error(f"Anthropic service '{self.name}' initialization failed: API key not found. Check that 'api_key' is set in configuration and ANTHROPIC_API_KEY environment variable is available.")
        ...
        try:
            ...
            raise LLMProviderError(f"Failed to initialize Anthropic service: {e}")
    ...
    async def chat_complete(self, messages: List[ChatMessage], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise LLMProviderAPIError(f"Anthropic API request failed: {e}")
        except KeyError as e:
            raise LLMProviderAPIError(f"Invalid Anthropic API response format: {e}")
    ...
    async def stream_chat_complete(self, messages: List[ChatMessage], **kwargs):
        ...
        try:
            ...
            raise LLMProviderAPIError(f"Anthropic streaming API request failed: {e}")
