        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> List[TextContent]:
            ...
            try:
                ...
                if registry.is_mcp_tool(name):
                    return [TextContent(
                        type="text",
                        text=f"Error: {name} is an external MCP tool, not available on this server"
                    )]
                ...
                from a2a.types import Task, Message, Part, TextPart, Role, TaskStatus, TaskState
                import uuid
                ...
                message = Message(
                    messageId=f"mcp_{uuid.uuid4().hex[:8]}",
                    role=Role.user,
                    parts=[part]
                )
